...
- **Live View wall** — a dedicated **Live View** page shows every live-capable camera at once in an NVR-style grid, with a selectable cameras-per-row layout (**Auto / 1× / 2× / 3× / 4×**, persisted) and a global quality toggle (defaults to the lighter sub stream); each tile links back to its camera page
