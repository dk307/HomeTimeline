...
def test_has_purgeable_camera(camera, tmp_path):
    ...
    cam.save()
    assert purger.has_purgeable_camera() is False
    # Re-enable but set retention to 0 (Never) → not purgeable.
    cam.enabled = True
