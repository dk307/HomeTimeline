...
def has_downloadable_camera() -> bool:
    """True if at least one enabled Hikvision camera with valid credentials exists
...
def download_all() -> dict[str, int]:
    """Download + index every enabled Hikvision camera with valid credentials. Cameras
    already downloading are skipped (their per-camera lock is held).
    Returns ``{camera_id: downloaded}``."""
